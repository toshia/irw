...
module Plugin::Irw
  class Setting < ::Gtk::CRUD
    ...
    def shortcutkeys
      (UserConfig[:remocon_keybinds] || Hash.new).dup end
    ...
    def on_created(iter)
      ...
      UserConfig[:remocon_keybinds] = bind
    end
    ...
    def popup_input_window(defaults = [])
      ...
      result = nil
      defaults.freeze
      window = KeyConfigWindow.new("設定 - %{software_name}" % {software_name: Environment::NAME})
    ...
    def key_box(results)
      ...
      label = Gtk::Label.new(results[COLUMN_KEYBIND] || '設定なし')
      remocon_tag = @plugin.handler_tag
      @plugin.on_irw_key_pushed(tags: remocon_tag) do |controller, key|
        results[COLUMN_KEYBIND] = "#{controller} #{key}"
        label.text = "#{controller} #{key}" unless label.destroyed?
      end
      label.ssc :destroy do
        @plugin.detach(remocon_tag)
        false
      end
      # container = ::Gtk::VBox.new(false, 16)
      # button = ::Plugin::Irw::IrConfig.new('リモコンボタン', results[COLUMN_KEYBIND])
    ...
    class CommandList < ::Gtk::TreeView
      ...
      def iter_match(iter, text)
        if ![COL_NAME, COL_SLUG].any?{ |column| iter[column].to_s.include?(text) } and iter.has_child?
